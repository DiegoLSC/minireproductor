                        ...
                        <div id="dropArtistas" class="accordion-collapse collapse" data-bs-parent="#acordeonSidebar">
                            <div class="accordion-body px-0 py-2">
                                ...
                                <div class="accordion accordion-flush" id="acordeonSubArtistas">
                                    ...
                                    <?php $collapseArtIdClean = $art['id']; ?>
                                    <?php $collapseArtId = 'collapse_art_' . $collapseArtIdClean; ?>
                                    <div class="accordion-item bg-transparent border-0 mb-1" id="block_art_<?= $art['id'] ?>">
                                        ...
                                        <div id="<?= $collapseArtId ?>" class="accordion-collapse collapse" data-bs-parent="#acordeonSubArtistas">
                                            <div class="accordion-body p-0 py-1 ps-4 ms-2 border-start border-secondary border-opacity-25">
                                                <ul class="list-unstyled d-flex flex-column gap-1 small mb-0" id="lista_albumes_art_<?= $art['id'] ?>">
                                                    <?php 
                                                    $tiene_albumes = false; 
                                                    foreach($albumes as $alb): 
                                                        if(in_array($art['id'], explode(',', $alb['artistas_ids'] ?? ''))): 
                                                            $tiene_albumes = true; 
                                                    ?>
                                                    <li class="d-flex align-items-center justify-content-between text-secondary p-1 ps-2 hover-bg-dark rounded item-con-opciones" id="li_alb_menu_<?= $alb['id'] ?>">
                                                        <div class="d-flex align-items-center gap-2 text-truncate flex-grow-1" style="cursor:pointer;" data-titulo="<?= htmlspecialchars(addslashes($alb['titulo']), ENT_QUOTES, 'UTF-8') ?>" onclick="document.getElementById('buscadorInput').value=this.getAttribute('data-titulo'); filtrarBiblioteca();">
