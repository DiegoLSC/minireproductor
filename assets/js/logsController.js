// ==========================================
// logsController.js - GESTIÓN DE AUDITORÍA
// Responsabilidad: Paginación y renderizado de logs del sistema.
// ==========================================
...
function cargarHistorialLogs() {
    ...
    const footerModal = document.querySelector('#modalLogs .modal-footer');
    if (!tbody) return;
    ...
    if (!contenedorBoton && footerModal) {
        ...
        contenedorBoton.className = 'w-100 text-center mb-2';
        footerModal.insertBefore(contenedorBoton, footerModal.firstChild);
    }
    ...
    fetch(`api/obtener_logs.php?offset=${logsOffset}`)
        ...
        .then(data => {
            if (data.estado === 'exito') {
                ...
                if (logsOffset === 0) tbody.innerHTML = `<tr><td colspan="4" class="text-center py-4 text-danger"><i class="bi bi-exclamation-triangle me-2"></i>Error: ${data.mensaje}</td></tr>`;
                if(contenedorBoton) contenedorBoton.innerHTML = `<span class="text-danger small">Error cargando más registros.</span>`;
            }
        })
        .catch(err => {
            console.error("Error obteniendo logs:", err);
            if (logsOffset === 0) tbody.innerHTML = `<tr><td colspan="4" class="text-center py-4 text-danger">Error de conexión al obtener el historial.</td></tr>`;
