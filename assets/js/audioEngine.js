// ==========================================
// audioEngine.js - CONTROLADOR DE MEDIOS Y COLA
// Responsabilidad: Reproducción, volumen real, sincronización de letras y visualizador.
// ==========================================
...
let volumenAnterior = parseFloat(localStorage.getItem('nebula_volumen')) || 1;
...
function inicializarAudio() {
    ...
    if (modoAleatorio && botonShuffleNativo) botonShuffleNativo.className = "btn text-success fs-5 p-0 lh-1";
    ...
    if (tituloGuardado && artistaGuardado) buscarLetra(artistaGuardado, tituloGuardado);
    ...
    document.body.addEventListener('click', function(e) {
        ...
        if (itemCola && perteneceACola) {
            ...
            if (!isNaN(indice)) reproducirDesdeCola(indice);
        }
    });
...
// 1. CONTROL DE ESTADO (Volumen y Play)
function toggleMute() {
...
audio.addEventListener('timeupdate', () => {
    if (audio.duration) {
        ...
        if(typeof formatearTiempo === 'function') timeCurrent.innerText = formatearTiempo(audio.currentTime);
...
audio.addEventListener('loadedmetadata', () => {
    if(typeof formatearTiempo === 'function') timeTotal.innerText = formatearTiempo(audio.duration);
});

audio.onended = function() {
    window.modoRepetirActivo ? (audio.currentTime = 0, audio.play()) : siguienteCancion();
};
...
function agregarAColaManual(rutaBuscada) {
    ...
    guardarEstadoCola();
    if (document.getElementById('colaPanel') && document.getElementById('colaPanel').classList.contains('activo')) renderizarColaVisual();
    ...
    try {
        ...
        if (!res.ok || res.status === 404) {
            ...
            if (arrayFallback && arrayFallback.length > 0) {
                renderizarDatosLetra(arrayFallback[0], contenedor);
                return;
            } else {
...
function renderizarLetras() {
    ...
    lyricsData.forEach((linea, index) => {
        ...
        p.onclick = () => {
            if(audio.duration) {
                audio.currentTime = linea.time;
                audio.play().catch(e => console.log("Play bloqueado", e));
                if (playBtn) playBtn.innerHTML = '<i class="bi bi-pause-fill fs-3 text-black"></i>';
            }
        };
...
function conectarFuenteAudio() {
    ...
    if (!sourceConectado && audio) {
        try {
            ...
            console.log("Visualizador ya conectado.");
        }
    }
...
function dibujarVisualizador() {
    ...
    for (let i = inicio; i < fin; i++) {
        const porcentajeAlto = dataArray[i] / 255;
        const alpha = porcentajeAlto * 0.8;
        const barHeight = porcentajeAlto * canvasVisualizador.height;
        ctxVisualizador.fillStyle = `rgba(${rgbSistema}, ${alpha})`;
